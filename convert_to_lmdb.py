...
parser.add_argument('-d', '--data_dir', default = 'C:\\Users\\sk_ma\\data', help='directory to SVHN (format 1) folders and write the converted files')
...
class ExampleReader(object):
    ...
    def _get_attrs(digit_struct_mat_file, index):
        ...
        i = Image.open(os.path.join(path,'%d.png' %(index + 1)))
        width, height = i.size
        ...
        for key in ['label', 'left', 'top', 'width', 'height']:
            ...
            if key == 'left' or 'width':
                attrs[key] = [x * 512 / width  for x in values]
            else:
                attrs[key] = [x * 512 / height  for x in values]
        return attrs
    ...
    def read_and_convert(self, digit_struct_mat_file):
        ...
        label_of_height = max(attrs['height'])
        label_of_left = max(attrs['left'])
        label_of_top = min(attrs['top'])
        label_of_width = max(attrs['width'])
        ...
        image = np.array(ExampleReader._preprocess(Image.open(path_to_image_file))).tobytes()

        example = example_pb2.Example()
        example.image = image
        example.height = label_of_height
        example.left = label_of_left
        example.top = label_of_top
        example.width = label_of_width
        return example
        ...
        with h5py.File(path_to_digit_struct_mat_file, 'r') as digit_struct_mat_file:
            ...
            for i in range(0, total_files, block_size):
                ...
                for offset in range(block_size):
                    ...
                    print(str(str_id))
                    txn.put(str_id.encode("utf-8"), example.SerializeToString())
                    num_examples[idx] += 1
